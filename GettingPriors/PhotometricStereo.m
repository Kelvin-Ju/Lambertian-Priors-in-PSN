...
uniqueMaskIndices = unique(maskIndex);%矩阵中唯一的原始
...
for iIdx = 1:length(uniqueMaskIndices)
  ...
  pixelIdx = find(maskIndex==idx);%find idx location
  ...
  Li = L(:, imageTag);%对应光照强度取出来
  % Create an M'xN' matrix of image intensities.
  Ii = I(imageTag, pixelIdx);%对应像素取出来
  % Compute the scaled normal.
  b(:, pixelIdx) = Li' \ Ii;%pinv(Li')*Ii
end
%nan的地方也可以直接给0
